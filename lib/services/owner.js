...
export async function updateBusinessTax(ownerId, { tax_rate, tax_label }) {
  const biz = await getOrCreateBusiness(ownerId);
  const rate = tax_rate === null || tax_rate === undefined || tax_rate === ""
    ? null
    : Number(tax_rate);
  if (rate !== null && (!Number.isFinite(rate) || rate < 0 || rate > 100))
    return { error: "tax_rate must be between 0 and 100", status: 400 };
  const label = (tax_label || "").trim() || null;
  const updated = await one(
    `UPDATE businesses
        SET tax_rate  = COALESCE($1, tax_rate),
            tax_label = COALESCE($2, tax_label)
...
export async function ownerUpdateBookingStatus(bookingId, ownerId, status) {
  ...
  // Issue review token + invoice the first time this booking completes.
